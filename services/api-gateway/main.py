...
@app.get("/projects/{project_id}/uml")
def list_project_uml(project_id: str):
    s3 = _minio_client()
    prefix = f"{project_id}/uml/"
    bucket = "artifacts"
    try:
        paginator = s3.get_paginator("list_objects_v2")
        files = []
        for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
            for obj in page.get("Contents") or []:
                ...
                if not name:
                    continue
                if name in {"diagrams.md", "signum.mdj"}:
                    files.append({"name": name, "url": f"s3://{bucket}/{key}"})
        if not files:
            return JSONResponse(status_code=404, content={"error": "No UML found"})
        return {"project_id": project_id, "uml": files}
    except (ClientError, EndpointConnectionError):
        return JSONResponse(status_code=500, content={"error": "Storage unavailable"})


@app.post("/projects/{project_id}/uml", status_code=202)
def build_project_uml(project_id: str, payload: Dict[str, Any] | None = None) -> JSONResponse:
    # Allow optional payload to enrich project info; ensure id from path is set
